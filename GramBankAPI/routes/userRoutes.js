...
router.post("/seed-test-users", async (req, res) => {
  try {
    const testUsers = [
      { name: "Aarav Sharma", phone: "9876543210", balance: 25000 },
      { name: "Vivaan Patel", phone: "9876543211", balance: 15000 },
      { name: "Aditya Kumar", phone: "9876543212", balance: 32000 },
      { name: "Vihaan Singh", phone: "9876543213", balance: 8500 },
      { name: "Arjun Reddy", phone: "9876543214", balance: 45000 },
      { name: "Sai Gupta", phone: "9876543215", balance: 12000 },
      { name: "Reyansh Verma", phone: "9876543216", balance: 28000 },
      { name: "Ayaan Joshi", phone: "9876543217", balance: 5500 },
      { name: "Krishna Iyer", phone: "9876543218", balance: 67000 },
      { name: "Ishaan Nair", phone: "9876543219", balance: 19500 },
      { name: "Ananya Das", phone: "9876543220", balance: 41000 },
      { name: "Diya Mehta", phone: "9876543221", balance: 9800 },
      { name: "Sara Rao", phone: "9876543222", balance: 23500 },
      { name: "Myra Pillai", phone: "9876543223", balance: 14200 },
      { name: "Kiara Desai", phone: "9876543224", balance: 37800 },
    ];

    const created = [];
    for (const u of testUsers) {
      const existing = await User.findOne({ phoneNumber: `+91${u.phone}` });
      if (existing) {
        created.push({ ...u, status: "already_exists" });
        continue;
      }

      const accountNumber = `${400000000000 + Math.floor(Math.random() * 9999999999)}`;
      const ifsc = `GRBN000${String(Math.floor(Math.random() * 99) + 1).padStart(2, "0")}`;
      const upiId = `${u.name.toLowerCase().replace(/\s+/g, ".")}@grambank`;
      ...
      const newUser = new User({
        ...
        aadhaarNumber: `${100000000000 + Math.floor(Math.random() * 899999999999)}`,
        accountNumber,
        ifsc,
        upiId,
        mpinHash,
      ...
      await newUser.save();
      created.push({ ...u, accountNumber, upiId, status: "created" });
    }

    res.json({ message: `Seed complete. ${created.filter(c => c.status === "created").length} new users created.`, users: created });
  } catch (err) {
