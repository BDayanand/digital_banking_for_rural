...
router.post("/create", auth, async (req, res) => {
  try {
    ...
    if (!txnDetails) {
      try {
        scheduledTxn = await ScheduledTransaction.findOne({
        ...
        console.log("Scheduled transaction not found");
      }
    }
    ...
    // BLOCKING REPORT TYPES: Cancel scheduled transaction immediately
    const BLOCKING_TYPES = ["UNAUTHORIZED", "WRONG_RECIPIENT", "FRAUD"];
    if (isScheduled && scheduledTxn && BLOCKING_TYPES.includes(report_type.toUpperCase()) && scheduledTxn.status === "PENDING") {
      ...
      return res.status(201).json({
        message: "Transaction report submitted. Scheduled transaction has been cancelled.",
