...
router.post("/scheduled-transactions/:txnId/process", async (req, res) => {
  try {
    ...
    const txn = await ScheduledTransaction.findById(txnId).populate("user_id");

    if (!txn) return res.status(404).json({ error: "Transaction not found" });
    if (txn.status !== "PENDING") return res.status(400).json({ error: "Transaction is not in PENDING state" });
    if (txn.processing_locked) return res.status(400).json({ error: "Transaction is locked" });
    if (txn.is_reported) return res.status(400).json({ error: "Transaction was reported and blocked" });
    ...
    if (action === "approve") {
      ...
      console.log(`[ADMIN] Transaction ${txn.txn_id} approved and completed by admin`);
    ...
    if (action === "reject") {
      ...
      console.log(`[ADMIN] Transaction ${txn.txn_id} rejected by admin`);
